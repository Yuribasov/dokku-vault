...
var helpEntries = []helpEntry{
	...
	{Name: "enable", Usage: "enable APP --mount-path PATH --role-name ROLE [--approle-mount PATH]", Summary: "Enable Vault rendering for a Dokku application.", Details: "APP must exist and use docker-local. Creates plugin-managed named storage mounted read-only in deploy and run containers. --approle-mount defaults to auth/approle."},
	...
	{Name: "template:add", Usage: "template:add APP NAME --secret-path PATH --field FIELD --destination FILE [--decode base64|none] [--perms MODE]", Summary: "Add a managed Vault KV v2 file template.", Details: "Required flags select the Vault path, data field, and relative output path. --decode defaults to base64. --perms defaults to 0444 and accepts 0400, 0440, or 0444."},
	...
	{Name: "stage", Usage: "stage APP --revision FULL_SHA --ttl-seconds N < wrapping-token-file", Summary: "Stage a wrapped AppRole SecretID for the next render.", Details: "Reads one response-wrapping token from stdin. FULL_SHA must be a full lowercase 40- or 64-character Git SHA. N must be between 1 and 86400. The credential is single-attempt."},
