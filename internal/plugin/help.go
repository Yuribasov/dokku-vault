...
var helpEntries = []helpEntry{
	...
	{Name: "stage", Usage: "stage APP (--revision FULL_SHA | --source-image IMAGE@sha256:DIGEST) --ttl-seconds N < wrapping-token-file", Summary: "Stage a wrapped AppRole SecretID for the next render.", Details: "Reads one response-wrapping token from stdin. Use --revision with a full lowercase 40- or 64-character Git SHA for Git deployments and their rebuilds. Use --source-image with the exact immutable application image passed to git:from-image or git:load-image, including later ps:rebuild operations for that image-origin app. N must be between 1 and 86400. The credential is single-attempt."},
